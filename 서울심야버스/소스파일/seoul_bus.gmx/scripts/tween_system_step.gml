...
for (var iiii=0, size = ds_list_size(g); iiii<size; iiii++) {
    ...
    if (_t[TLTWEEN.KILLMEPLEASE]) {
        ...
        if (_t[TLTWEEN.PLAY] && dir!=0) {
            ...
            var timetorender = time;
            if (time<delay && timecache>=delay) {
                timetorender=delay;
            }
            if (time>timeTweenEnd && timecache<=timeTweenEnd) {
                timetorender=timeTweenEnd;
            }
            if (timetorender>=delay && timetorender<=timeTweenEnd) {
